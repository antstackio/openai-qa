...
```bash
openai-qa$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
openai-qa$ sam local start-api
openai-qa$ curl http://localhost:3000/classify_digit
```
...
```bash
openai-qa$ sam logs -n InferenceFunction --stack-name openai-qa --tail
```
...
```bash
aws cloudformation delete-stack --stack-name openai-qa
```
